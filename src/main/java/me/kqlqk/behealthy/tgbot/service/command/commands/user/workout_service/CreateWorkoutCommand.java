...
public class CreateWorkoutCommand implements Command {
    ...
    @Autowired
    public CreateWorkoutCommand(TelegramUserService telegramUserService, GatewayClient gatewayClient) {
    ...
    public void handle(Update update, TelegramUser tgUser, TokensDTO tokensDTO, SecurityState securityState) {
        ...
        if (tgUser.getCommandSate() == CommandState.BASIC) {
            String text = "Let's create your workout plan.\nEnter how many times per week you can go to the gym (from 1 to 5)";
            sendMessage = new SendMessage(chatId, text);

            tgUser.setCommandSate(CommandState.CREATE_WORKOUT_WAIT_FOR_DATA);
            telegramUserService.update(tgUser);
            return;
        }
        else if (tgUser.getCommandSate() == CommandState.CREATE_WORKOUT_WAIT_FOR_DATA) {
            int workoutsPerWeek;
            try {
            ...
            try {
                gatewayClient.createWorkout(tgUser.getUserId(), workoutInfoDTO, tokensDTO.getAccessToken());
            }
            ...
            sendMessage = new SendMessage(chatId, "Workout plan was successfully created");
            return;
        }
